...
while :; do
    certbot renew --non-interactive --no-self-upgrade --dns-cloudflare --dns-cloudflare-propagation-seconds 60 --dns-cloudflare-credentials /cloudflare.key --email certs@trenton.io --agree-tos --server https://acme-staging-v02.api.letsencrypt.org/directory -d "$CERTBOT_SERVER_NAME" # https://acme-v02.api.letsencrypt.org/directory
    sleep 12h & wait $${!}
done
git gc --prune=now
